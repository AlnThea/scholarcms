...
export default function PageClient({ params, initialPage, initialSiteTitle }) {
  const { slug } = params;
  ...
  useEffect(() => {
    if (!initialPage) {
      ...
      const displayTitle = translateLabel(initialPage.title, language);
      document.title = siteTitle ? `${displayTitle} - ${siteTitle}` : displayTitle;
    }
  }, [slug, language, initialPage]);
