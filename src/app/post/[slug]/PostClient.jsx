...
export default function BlogPostDetail({ 
  ...
  const [siteTitle, setSiteTitle] = useState(initialSiteTitle || 'ByteLab');
  const [comments, setComments] = useState(initialComments || []);
  const [loading, setLoading] = useState(initialPost ? false : true);
  const [copied, setCopied] = useState(false);

  useEffect(() => {
    // Setup prev and next posts if we have initial data
    if (initialAllPosts && initialAllPosts.length > 0 && initialPost) {
      const idx = initialAllPosts.findIndex(p => p.id === initialPost.id || p.slug === initialPost.slug);
      if (idx !== -1) {
        setPrevPost(idx > 0 ? initialAllPosts[idx - 1] : null);
        setNextPost(idx < initialAllPosts.length - 1 ? initialAllPosts[idx + 1] : null);
      }
      
      const currentSiteTitle = initialSiteTitle || 'ByteLab';
      if (typeof document !== 'undefined') {
        document.title = currentSiteTitle ? `${initialPost.title} - ${currentSiteTitle}` : initialPost.title;
      }
    }
    ...
    loadPostAndDetails();
  }, [slug, initialPost, initialAllPosts, initialSiteTitle]);
